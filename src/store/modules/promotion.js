...
const mutations = {
  storePromotionsRequest: state => {
    (state.loading = true), (state.data = null), (state.err = null);
  },

  storePromotionsSuccess: (state, payload) => {
    (state.loading = false), (state.data = payload), (state.err = null);
  },

  storePromotionsFailed: (state, payload) => {
    (state.loading = false), (state.data = null), (state.err = payload);
  }
};
...
const actions = {
  ...
  fetchEditPromotion({ commit }, promotion) {
    commit("storePromotionsRequest");
    api
      .patch(`/promotions/${promotion._id}`, promotion.promotion)
      .then(res => {
        commit("storePromotionsSuccess", res.data);
